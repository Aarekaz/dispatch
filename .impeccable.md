...
- **Generic shadcn dashboard demo** — stat-card row, identical 2-col grid, repeating cards, lucide icons in every header. This is what the current home page is dangerously close to and must be moved away from
...
**Color:** Pure neutral OKLCH grays as the foundation (already established). Accent color is reserved for actual semantic meaning — status (success / warning / destructive / info) and never for decoration. There is no brand hue. The neutrality IS the brand.

**Type:** Geist Sans is the workhorse. Geist Mono only for actual code/IDs. Newsreader serif (already loaded) is available for editorial moments — large display headings, empty-state hero text — but used sparingly. Body text is comfortable, not dense.
