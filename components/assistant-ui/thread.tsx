    ...
    <ComposerPrimitive.Root className="aui-composer-root relative flex w-full flex-col">
      <ComposerPrimitive.AttachmentDropzone asChild>
        <div
          ...
          className="flex w-full flex-col gap-2 rounded-(--composer-radius) border border-border/50 bg-background p-(--composer-padding) transition-colors duration-150 focus-within:border-border focus-within:ring-2 focus-within:ring-ring/20 data-[dragging=true]:border-ring data-[dragging=true]:border-dashed data-[dragging=true]:bg-accent/50"
        >
          <ComposerAttachments />
...
/**
 * Collapsible reasoning/thinking block. Streams live while the model thinks
 * (expanded by default). Users can click to collapse. Renders the part's
 * accumulated `text` directly — the runtime updates it on every delta.
 *
 * `state === "done"` signals history-loaded reasoning (set by selectSession)
 * so we default to collapsed to avoid burying old conversations under walls
 * of thinking text. Live streaming parts have no state or state="streaming"
 * and default to expanded so the user can watch the model think in real time.
 */
const ReasoningBlock: FC<{ text: string; state?: "streaming" | "done" }> = ({
  text,
  state,
}) => {
    ...
    >
      <div className="aui-assistant-message-content wrap-break-word px-2 text-foreground leading-relaxed">
        <MessagePrimitive.Parts>
          {({ part }) => {
            ...
            if (part.type === "tool-call") {
              // Registered makeAssistantToolUI components provide part.toolUI automatically.
              // ToolFallback handles any unregistered tools.
        ...
        <AuiIf condition={(s) => s.message.isLast && s.thread.isRunning}>
          <div className="mt-2"><Spinner context="thinking" label="Thinking…" className="text-sm text-muted-foreground" /></div>
        </AuiIf>
        <MessageError />
      </div>
...
const UserMessage: FC = () => {
  return (
    ...
    >
      ...
      <div className="aui-user-message-content-wrapper group relative w-full">
        <div className="aui-user-message-content wrap-break-word ml-auto w-fit max-w-[80%] rounded-tl-2xl rounded-tr-2xl rounded-bl-2xl rounded-br-sm bg-muted px-4 py-2.5 text-foreground empty:hidden">
          <MessagePrimitive.Parts />
        </div>
