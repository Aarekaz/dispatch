...
export const TodoTool = memo(function TodoTool({
  ...
  const output = (part.output ?? {}) as { oldTodos?: TodoItem[]; newTodos?: TodoItem[] };
  const input = (part.input ?? {}) as { todos?: TodoItem[] };
  const oldTodos = output.oldTodos ?? EMPTY_TODOS;
  const newTodos = input.todos ?? output.newTodos ?? EMPTY_TODOS;
