...
export const MessageList = memo(function MessageList({
  ...
  const [assistantSpaceActive, setAssistantSpaceActive] = useState(false);
  const [lastSeenAssistantMessageId, setLastSeenAssistantMessageId] = useState<
    string | null
  >(messages[messages.length - 1]?.id ?? null);
  ...
  const isNewAssistantMessage =
    ...
    lastMessageId !== lastSeenAssistantMessageId;
  const showAssistantBreathingSpace =
    showPlanning || assistantSpaceActive || isNewAssistantMessage;
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      if (lastMessageRole === "assistant") {
        if (lastMessageId && lastMessageId !== lastSeenAssistantMessageId) {
          setAssistantSpaceActive(true);
        }
      }
      if (lastMessageRole === "user") {
        setAssistantSpaceActive(false);
      }
      setLastSeenAssistantMessageId(lastMessageId);
      lastMessageIdRef.current = lastMessageId;
    });
    return () => {
      cancelled = true;
    };
  }, [lastMessageId, lastMessageRole, lastSeenAssistantMessageId]);
