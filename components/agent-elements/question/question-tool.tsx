...
export function QuestionTool({ part }: QuestionToolProps) {
  ...
  const controlledQuestionIndex = part.input?.questionIndex;
  ...
  useEffect(() => {
    const nextIndex = controlledQuestionIndex;
    if (typeof nextIndex !== "number") return;
    let cancelled = false;
    queueMicrotask(() => {
      if (!cancelled) setLocalIndex(nextIndex);
    });
    return () => {
      cancelled = true;
    };
  }, [controlledQuestionIndex]);

  useEffect(() => {
    const nextIndex = controlledQuestionIndex ?? 1;
    let cancelled = false;
    queueMicrotask(() => {
      if (cancelled) return;
      setLocalAnswers({});
  ...
  }, [controlledQuestionIndex, part.toolCallId]);
